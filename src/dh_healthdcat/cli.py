...
from dh_healthdcat.selection import TagMode, select_data_product_urns
from dh_healthdcat.validate.shacl import ShaclResult, validate_graph

if TYPE_CHECKING:
    from dh_healthdcat.config import Resolved
...
def _echo_shacl_violation(dataset: HealthDataset, shacl: ShaclResult) -> None:
    if not shacl.conforms:
        typer.secho(
            f'DataProduct "{dataset.title}" : non conforme SHACL', fg=typer.colors.RED, err=True
        )
    ...
    *,
    ...
    try:
        config_path = config_module.find_config_file(
            explicit=cli_config, env=os.environ, cwd=Path.cwd(), home=Path.home()
        )
        ...
        return config_module.resolve_target(
            ...
            cli_api_key_env=cli_api_key_env,
        )
...
def _select_or_exit(
    ...
    *,
    ...
    if not selected:
        typer.secho(
            "Aucun DataProduct ne correspond aux filtres.", fg=typer.colors.YELLOW, err=True
        )
...
def export_file(
    output: Path = typer.Option(
        ..., "--output", "-o", help="Fichier ou dossier de sortie (voir --split-per-dataset)"
    ),
    urn: list[str] = typer.Option(
        [],
        "--urn",
        help=(
            "URN de DataProduct à exporter (répétable). Sans valeur : tout le "
            "catalogue. Prioritaire sur --domain/--tag/--exclude-tag."
        ),
    ),
    domain: list[str] = typer.Option(
        [], "--domain", help="Filtre par domaine (urn ou id, répétable)"
    ),
    tag: list[str] = typer.Option([], "--tag", help="Filtre par tag (urn ou nom, répétable)"),
    tag_mode: str = typer.Option(
        "any", "--tag-mode", help="any (au moins un des --tag, défaut) | all (tous les --tag)"
    ),
    exclude_tag: list[str] = typer.Option(
        [], "--exclude-tag", help="Exclut les DataProducts portant ce tag (urn ou nom, répétable)"
    ),
    fmt: str = typer.Option("turtle", "--format", help="turtle | json-ld | n-triples"),
    split_per_dataset: bool = typer.Option(
        False, "--split-per-dataset", help="Un fichier par DataProduct dans --output (dossier)"
    ),
    base_uri: str = typer.Option(DEFAULT_BASE_URI, "--base-uri", help="Base pour dct:identifier"),
    strict: bool = typer.Option(
        True,
        "--strict/--no-strict",
        help="Échoue si un jeu a une ERREUR SHACL (désactiver pour un export exploratoire)",
    ),
) -> None:
    ...
    selected = _select_or_exit(
        ctx, urn=urn, domain=domain, tag=tag, tag_mode=tag_mode, exclude_tag=exclude_tag
    )
    ...
    for outcome in pipeline.prepare(ctx, selected, base_uri=base_uri, strict=strict):
        ...
        if isinstance(outcome, pipeline.Rejected):
            typer.secho(
                f'  -> "{dataset.title}" exclu de l\'export : erreurs ci-dessus '
                "(--no-strict pour forcer)",
    ...
    if split_per_dataset:
        ...
        if not graphs:
            typer.secho(
                "Export non écrit : aucun DataProduct valide dans la sélection "
                "(--no-strict pour forcer).",
                fg=typer.colors.RED,
                err=True,
            )
        ...
        typer.echo(
            f"Écrit : {output} ({len(merged)} triplets, "
            f"{len(graphs)}/{len(selected)} jeu(x){suffix})"
        )
...
def validate(
    ...
    fmt: str | None = typer.Option(
        None,
        "--format",
        help="Format RDF (turtle|json-ld|n-triples). Déduit de l'extension si omis.",
    ),
) -> None:
...
def push_hdh(
    hdh_url: str | None = typer.Option(
        None,
        "--hdh-url",
        help="Base URL du catalogue (prioritaire sur $HDH_URL et le profil sélectionné)",
    ),
    profile: str | None = typer.Option(
        None,
        "--profile",
        help=(
            "Profil déclaré dans le fichier de configuration "
            "(sinon $HDH_PROFILE, default_profile, ou l'unique profil)"
        ),
    ),
    config: Path | None = typer.Option(
        ...
        help=(
            ...
            "(sinon $HDH_CONFIG, ./.dh-healthdcat.yml, ~/.dh-healthdcat.yml)"
        ),
    ),
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="Clé mdc_... (sinon lue depuis la variable résolue par --api-key-env)",
    ),
    api_key_env: str | None = typer.Option(
        None,
        "--api-key-env",
        help=(
            "Variable d'environnement portant la clé API (sinon celle du profil, sinon HDH_API_KEY)"
        ),
    ),
    urn: list[str] = typer.Option(
        [],
        "--urn",
        help=(
            "URN de DataProduct à pousser (répétable). Sans valeur : tout le "
            "catalogue. Prioritaire sur --domain/--tag/--exclude-tag."
        ),
    ),
    domain: list[str] = typer.Option(
    ...
    state_file: Path | None = typer.Option(
        None,
        "--state-file",
        help="Fichier de correspondance URN->id HDH (défaut : .dh-healthdcat-state.json)",
    ),
    dry_run: bool = typer.Option(
        False,
        "--dry-run",
        help="Valide et affiche ce qui serait envoyé, sans appel réseau d'écriture",
    ),
) -> None:
    ...
    resolved = _resolve_target_or_exit(
        cli_url=hdh_url, cli_profile=profile, cli_config=config, cli_api_key_env=api_key_env
    )

    resolved_key = api_key or os.environ.get(resolved.api_key_env)
    if not resolved_key:
        typer.secho(
            f"Aucune clé API : passez --api-key ou définissez ${resolved.api_key_env}.",
            fg=typer.colors.RED,
            err=True,
        )
        raise typer.Exit(code=1)
    ...
    try:
        ...
        typer.secho(
            f"Clé API invalide ou rôle data-provider manquant : {exc}",
            fg=typer.colors.RED,
            err=True,
        )
    ...
    state = state_module.PushState.open(
        state_path,
        instance=resolved.url,
        on_warning=lambda m: typer.secho(m, fg=typer.colors.YELLOW, err=True),
    )
    any_failure = False

    for outcome in pipeline.push(
        ctx, selected, target=client, state=state, base_uri=base_uri, dry_run=dry_run
    ):
        if isinstance(outcome, pipeline.Unreadable):
            typer.secho(f"IGNORÉ {outcome.urn} : {outcome.reason}", fg=typer.colors.RED, err=True)
            any_failure = True
        ...
        if isinstance(outcome, pipeline.Rejected):
            typer.secho(
                f'IGNORÉ "{dataset.title}" : invalide '
                "(SHACL et/ou champs obligatoires manquants, voir ci-dessus)",
                fg=typer.colors.RED,
                err=True,
            )
            if not outcome.shacl.conforms:
        ...
        if isinstance(outcome, pipeline.Planned):
            action = (
                f"PUT (mise à jour de {outcome.existing_id})"
                if outcome.existing_id
                else "POST (création)"
            )
            typer.echo(
                f'[dry-run] "{dataset.title}" -> {action} · {outcome.turtle_bytes} octets Turtle'
            )
            continue
