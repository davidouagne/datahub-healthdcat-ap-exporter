...
| `PersonalData.yml` : mélange `dpv-pd#`/`pd#` + entrée `Death` sans terme DPV correspondant | Écarts confirmés présents à l'identique dans la source HDH (fichier vendu) le 2026-09-18, non corrigés localement (« HDH fait foi ») — voir `vocab/README.md` et issues #85/#86 | à signaler côté HDH pour resynchronisation |
