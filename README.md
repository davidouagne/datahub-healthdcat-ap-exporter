...
Implémentés et testés : le mapping, l'export fichier, la poussée API avec idempotence, et la configuration de l'instance par profils avec état cloisonné (voir « Configuration de l'instance » ci-dessus). Reste à faire avant un usage en production :

- **Curation** — peupler les propriétés `fr.aphp.healthdcat.*` sur les DataProducts existants (`aphp/datahub-sample`). Aujourd'hui la quasi-totalité en sont dépourvus et sont donc exclus de tout export/poussée strict (`dct:accessRights`, `dct:publisher`, `dcat:distribution`... manquants). Deux DataProducts de test entièrement conformes (`test-healthdcat-ap-sample`/`test-healthdcat-ap-patient`, dans `dataproduct-layer/assets.yml`) servent de référence : le connecteur d'ingestion réel (`aphp/datahub-yaml-source`) les ingère, `dh-healthdcat` les relit et les valide conformes (SHACL) — voir `docs/mapping.md`.
- **Poussée réelle vers un catalogue HDH** — un test contre une instance HDH locale (2026-08-16) montre que `POST /ingest/datasets` répond systématiquement 500 (« Internal Server Error » générique), y compris pour un Turtle trivial sans rapport avec HealthDCAT-AP et en contournant entièrement `dh-healthdcat` (`curl` direct) : le défaut est côté serveur HDH, pas dans ce dépôt. À faire suivre auprès de l'équipe HDH avant de considérer `push-hdh` validé de bout en bout contre une instance réelle.
